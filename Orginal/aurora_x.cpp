...
struct Intention {
  ...
  int    ris_tiles=16;
  static Intention parse(const string& s){
    Intention I; auto tok=[&](string x){ vector<string>v; string cur; for(char c:x){ if(c==';'){ if(!cur.empty()) v.push_back(cur); cur.clear(); } else cur.push_back(c);} if(!cur.empty()) v.push_back(cur); return v;};
    for(auto& kv: tok(s)){ vector<string> p; string a; for(char c:kv){ if(c==':'){ p.push_back(a); a.clear(); } else a.push_back(c);} p.push_back(a);
      if(p.size()!=2) continue; auto k=p[0], v=p[1]; auto num=[&](){return stod(v);};
      if(k=="deadline") I.deadline_s=num();
      else if(k=="reliability") I.reliability=num();
      else if(k=="duty") I.duty_frac=num();
      else if(k=="optical") I.allow_optical=(v=="on");
      else if(k=="backscatter") I.allow_backscatter=(v=="on");
      else if(k=="ris") I.ris_tiles=max(0, stoi(v));
    } return I;
  }
};
