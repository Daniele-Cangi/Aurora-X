...
private:
    ...
    void update_flow_state(
        ...
        if (!delivered) {
            // Aumento difese di base
            st.crit_overhead += alpha_up;
            st.bulk_overhead += alpha_up;
            ...
            if (profile.flow_class == FlowClass::NERVE ||
                ...
                // Booster extra immediato
                st.crit_overhead += alpha_up;   // doppio impatto sul critico
                
                // Se i fallimenti sono ripetuti, aumenta ancora un po' (shock prolungato)
                if (st.bad_streak >= 3) {
                    st.crit_overhead += alpha_up * 0.5;
                    st.bulk_overhead += alpha_up * 0.5;
                }
            }
        }
        ...
        // Non saliamo oltre il massimo consentito
        st.crit_overhead = std::clamp(st.crit_overhead, MIN_OV, MAX_OV);
        st.bulk_overhead = std::clamp(st.bulk_overhead, MIN_OV, MAX_OV);
